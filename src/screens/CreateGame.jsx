...
export default function CreateGame() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState(null) // player to delete
  ...
  async function createGame() {
    ...
    try {
      const { data: game, error: gameError } = await supabase
        .from('games').insert({ title: title.trim() || 'ערב פוקר' }).select().single()
    ...
    <div className="screen">
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ color: 'var(--text2)', marginBottom: 20 }}>
              ...
              <span style={{ fontSize: '0.82rem', color: 'var(--text3)' }}>
                ניתן למחוק רק שחקנים שלא השתתפו במשחקים.
              </span>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button className="btn btn-danger" style={{ flex: 1 }} onClick={() => deletePlayer(deleteConfirm)}>
                מחק
              </button>
      ...
      {resetConfirm && (
        <div className="modal-overlay" onClick={() => setResetConfirm(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-title" style={{ color: 'var(--red)' }}>
              <AlertTriangle size={18} /> איפוס כל הנתונים
            </div>
            <div style={{
